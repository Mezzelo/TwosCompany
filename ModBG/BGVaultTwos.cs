...
namespace TwosCompany.ModBG {
    public class BGVaultTwos : BG {
        ...
        public void DrawVaultBg(G g, double t, Vec lookAway = default(Vec), bool showOverlook = false, bool extraDark = false, bool showShips = false, bool letterbox = false) {
            ...
            if (showOverlook) {
                for (int i = 0; i < 6; i++) {
                    Spr? id = Spr.bg_shipyard_scaffolding;
                ...
                for (int j = 0; j < 6; j++) {
                    Spr? id2 = Spr.bg_shipyard_scaffolding;
            ...
            // if (!cobaltExploded)
            {
                Spr? id6 = Spr.bg_cobalt_hero_front;
                double x5 = vec.x;
                double y4 = vec.y + 0.0;
            ...
            if (showOverlook) {
                ...
                Spr? id7 = Spr.bg_shipyard_platform;
        ...
        public static void DrawMediumIsosphere(G g, Vec sunPos) {
            if (Mutil.Rand((int)(g.state.time * 5.0)) < 0.8) {
                SpriteUtil.GlitchSprite(Spr.effects_icosphere, (int)sunPos.x - 85, (int)sunPos.y - 85, 240.0, new Vec(170.0, 170.0), (int)Math.Round(g.state.time * 10.0), 0.8, new Color(0.0, 0.2, 1.0).gain(1.0), BlendMode.Screen);
                return;
            }
            Spr? id = Spr.effects_icosphere;
            double x = sunPos.x;
            double y = sunPos.y;
        ...
        public static void DrawDigitalCameraNoise(G g, double duty = 0.1, Color? color = null) {
            SpriteUtil.GlitchSprite(Spr.effects_glitch, 0, 0, 240.0, new Vec(480.0, 270.0), (int)Math.Round(g.state.time * 3.0), duty, color ?? new Color(0.0, 0.2, 1.0).gain(0.2), BlendMode.Screen);
        }
